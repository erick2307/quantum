...
class QLearning:
    def __init__(
        ...
        self.populationAtLinks = np.zeros(
            (self.linksdb.shape[0], 2)
        ...
        self.popHistPerLink = np.zeros(
            (self.linksdb.shape[0], int(max(self.popAtLink_HistParam[:, 1])) + 1)
        )
        # 2020Oct07: Memory for density array at links+1
        self.denArrPerLink = np.zeros(
            (self.linksdb.shape[0], int(max(self.popAtLink_HistParam[:, 1])) + 1)
        )
        # 2020Oct07: Memory for velocity array at links
        self.speArrPerLink = np.zeros(
            (self.linksdb.shape[0], int(max(self.popAtLink_HistParam[:, 1])) + 1)
        )

        # for p in self.popHistPerLink: print(p)
        self.transLinkdb = np.loadtxt(
            transLinkdbFile, delimiter=",", dtype=np.int
        )  # database of actions [currentNode, numberOfNodesTarget, linkConnectingNode1, linkConnectingNode2,...]
        self.transNodedb = np.loadtxt(
            transNodedbFile, delimiter=",", dtype=np.int
        ...
        self.pedProfiles = np.loadtxt(
            agentsProfileName, delimiter=",", dtype=np.int
        )  # agents profile [age, gender, householdType, householdId, closestNodeNumber]
        ...
        self.pedDB[:, 9] = np.round(
            np.random.rayleigh(scale=scaleRayleigh, size=self.pedDB.shape[0]),
            decimals=0,
        ).astype(np.int)
        ...
        for i in range(self.nodesdb.shape[0]):
            self.stateMat[i, 11 : 11 + int(self.transLinkdb[i, 1])] = 0.5 * np.ones(
                self.transLinkdb[i, 1]
            )
            self.stateMat[i, 21 : 21 + int(self.transLinkdb[i, 1])] = np.ones(
                self.transLinkdb[i, 1]
            )
    ...
    def computePedHistDenVelAtLinks(self):
        ...
        for ind in occupLinksIndx:
            ...
            self.speArrPerLink[ind, :numComp] = (
                1.388 - 0.396 * self.denArrPerLink[ind, :numComp]
            )
            self.speArrPerLink[ind, :numComp] = np.clip(
                self.speArrPerLink[ind, :numComp], 0.2, 1.19
            )
        return
    ...
    def computeDensityLevel(self, codeLink, linkWidth=2.0):
        """
        Computes the pedestrian-density level at specific link.
        The current function has only three levels of pedestrian-density
        """
        density = float(self.populationAtLinks[codeLink, 1]) / (
            linkWidth * self.linksdb[codeLink, 3]
        )
    ...
    def getStateIndexAtNode(self, codeNode):
        ...
        self.stateMat = np.concatenate(
            (self.stateMat, state_arr.reshape(1, 31)), axis=0
        )
    ...
    def checkTarget(self, ifOptChoice=False):
        ...
        error = (
            (self.pedDB[:, 0] - self.pedDB[:, 2]) ** 2
            + (self.pedDB[:, 1] - self.pedDB[:, 3]) ** 2
        ) ** 0.5
    ...
    def updateSpeed(self, codeLink, linkWidth=2.0):
        """
        Computes the speed at link "codeLink" considering its actual pedestrian-density.
        2021Jan06(E) we need to consider vehicles instead of pedestrians.
        A new function was copied with different speeds.
        """
        # Computes the density at link
        density = self.populationAtLinks[codeLink, 1] / (
            linkWidth * self.linksdb[codeLink, 3]
        )
        # Assign a speed according the the density
        # Check paper reference, where these values come from
    ...
    def updateVelocityV1(self, pedIndx, linkWidth=2.0):
        """
        ...
        unitDir = (self.pedDB[pedIndx, 2:4] - self.pedDB[pedIndx, :2]) / np.linalg.norm(
            self.pedDB[pedIndx, 2:4] - self.pedDB[pedIndx, :2]
        )
    ...
    def updateVelocityV2(self, pedIndx):
        ...
        if codeLink == -1:
            ...
            unitDir = (
                self.pedDB[pedIndx, 2:4] - self.pedDB[pedIndx, :2]
            ) / np.linalg.norm(self.pedDB[pedIndx, 2:4] - self.pedDB[pedIndx, :2])
            vel_arr = speed * unitDir
            self.pedDB[pedIndx, 4:6] = vel_arr
        return
    ...
    def initEvacuationAtTime(self):
        ...
        if len(indxPed) != 0:
            for i in indxPed:
                node0 = int(self.pedDB[i, 8])  # current node
                indxTgt = np.random.choice(
                    int(self.transNodedb[node0, 1])
                )  # random choice for the next node
                nodeTgt = self.transNodedb[node0, 2 + indxTgt]  # next number node
                link = self.transLinkdb[node0, 2 + indxTgt]  # next link code
                self.pedDB[i, 7] = nodeTgt
                self.pedDB[i, 6] = link
                self.pedDB[i, 2:4] = self.nodesdb[
                    nodeTgt, 1:3
                ]  # coordinates of the next target (next node)
    ...
    def updateTarget(self, pedIndx, ifOptChoice=False):
        ...
        # Otherwise, we assign new target node:
            ...
            Qval_arr = self.stateMat[
                stateIndx, 11 : 11 + int(self.transLinkdb[node0, 1])
            ]
            ...
            # updated:
                ...
                xTgt_arr = np.array(
                    [self.nodesdb[nodeTgt, 1], self.nodesdb[nodeTgt, 2]]
                )
    ...
    def tdControl(self, pedIndx, alpha=0.05):
        ...
        if self.pedDB[pedIndx, 10]:
            ...
            self.stateMat[current_S, 11 + current_A] += alpha * (
        ...
        self.stateMat[pre_S, 11 + pre_A] += alpha * (
            preReward
            + self.discount * self.stateMat[current_S, 11 + current_A]
    ...
    def loadShortestPathDB(self, namefile):
        self.shortestPathDB = np.loadtxt(
            namefile, delimiter=",", skiprows=1, dtype=np.int
        )
    ...
    def updateValuefunctionByAgent(self, pedIndx):
        ...
        self.stateMat[int(expSta[0, 0]), int(11 + expSta[0, 1])] += (
            reward - self.stateMat[int(expSta[0, 0]), int(11 + expSta[0, 1])]
        ...
        if len(expSta) > 1:
            for i in range(1, expSta.shape[0]):
                if expSta[i, 0] in expSta[:i, 0]:
                    continue
                self.stateMat[int(expSta[i, 0]), 11 + int(expSta[i, 1])] += (
                    reward - self.stateMat[int(expSta[i, 0]), 11 + int(expSta[i, 1])]
    ...
    def updateValuefunctionByAgentV2(self, pedIndx, ifConstStepSize=True, alpha=0.05):
        ...
        if ifConstStepSize:
            self.stateMat[state, 11 + choice] += alpha * (
                G - self.stateMat[state, 11 + choice]
            )
            self.stateMat[state, 21 + choice] += 1
        else:
            self.stateMat[state, 11 + choice] += (
                G - self.stateMat[state, 11 + choice]
            ) / (self.stateMat[state, 21 + choice] + 1)
            self.stateMat[state, 21 + choice] += 1
    ...
    def computeAction_Value_Policy(self):
        ...
        # Get value function
        VFun = np.sum(
            self.stateMat[:, 11:21] * self.stateMat[:, 21:31], axis=1
        ) / np.sum(self.stateMat[:, 21:31], axis=1)
    ...
    def setFigureCanvas(self):
        ...
        for i in range(self.linksdb.shape[0]):
            self.ax.plot(
                [
                    self.nodesdb[int(self.linksdb[i, 1]), 1],
                    self.nodesdb[int(self.linksdb[i, 2]), 1],
                ],
                [
                    self.nodesdb[int(self.linksdb[i, 1]), 2],
                    self.nodesdb[int(self.linksdb[i, 2]), 2],
                ],
                c="k",
                lw=1,
            )
        ...
        (self.p1,) = self.ax.plot(
            self.nodesdb[indxEv, 1],
            self.nodesdb[indxEv, 2],
            "rD",
            ms=4,
            mfc="none",
            mec="r",
    ...
    def getSnapshotV2(self, area):
        ...
        self.labelTime = self.fig.text(
            0,
            0,
            "t = %.2f min; evacuated: %d of %d"
    ...
    def getSnapshotV3(self, area, outputfolder):
        ...
        gdf = gpd.read_file(f"{area}/ref/aos.geojson", driver="GeoJSON").to_crs(
            par.PJ_CRS
        )
        ctx.add_basemap(
            self.ax,
        ...
        self.fig.savefig(
            os.path.join(outputfolder, "figures", "Figure_%04d.png" % self.snapshotNumber),
            bbox_inches="tight",
            dpi=150,
        )
        self.snapshotNumber += 1
    ...
    def plotNetwork(self):
        ...
        indxZeroActions = np.where(self.transLinkdb[:, 1] != 0)[0]
        plt.scatter(
            self.nodesdb[indxZeroActions][:, 1],
            self.nodesdb[indxZeroActions][:, 2],
            s=20,
            edgecolor="k",
            linewidths=0.0,
        )
        plt.scatter(
            self.pedDB[: self.numPedestrian, 0],
            self.pedDB[: self.numPedestrian, 1],
            s=40,
            c="r",
            linewidth=0.0,
        )
