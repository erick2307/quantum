...
def setMatrices(numcolumns=12):
    nodesdb = np.loadtxt(
        Path(par.CASE_FOLDER, "nodesdb.csv"), delimiter=",", skiprows=1
    )
    linksdb = np.loadtxt(
        Path(par.CASE_FOLDER, "linksdb.csv"), delimiter=",", skiprows=1
    )
    ...
    for i in range(numNodes):
        ...
        if numlinks2:
            ...
            transitionsdb[i, 2 + numlinks1 : 2 + numlinks1 + numlinks2] = tmpLinksdb2[
                :, 1
            ]
    ...
    #    print(probdb[ind])
    np.savetxt(
        Path(par.CASE_FOLDER, "actionsdb.csv"), actionsdb, delimiter=",", fmt="%d"
    )
    np.savetxt(
        Path(par.CASE_FOLDER, "transitionsdb.csv"),
        transitionsdb,
        delimiter=",",
        fmt="%d",
    )
