...
def plotNetwork():
    noDB = np.loadtxt(Path(par.CASE_FOLDER, "nodesdb0.csv"), delimiter=",")
    edDB = np.loadtxt(Path(par.CASE_FOLDER, "linksdb0.csv"), delimiter=",")
...
def main():
    ...
    iShpPath = str(Path(par.CASE_FOLDER, "edges.shp"))
    noDB, edDB = readEdges(iShpPath)
    np.savetxt(
        Path(par.CASE_FOLDER, "nodesdb0.csv"), noDB, fmt="%d,%.2f,%.2f", delimiter=","
    )
    np.savetxt(Path(par.CASE_FOLDER, "linksdb0.csv"), edDB, fmt="%d", delimiter=",")
    return
